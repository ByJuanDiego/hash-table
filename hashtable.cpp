//
// Created by juandiego on 3/21/23.
//
...
template<typename K, typename V, typename Hash, typename Index, typename Equal, int RehashingFactor>
requires RehashingFactorConstraint<RehashingFactor>std::list<V>
...
template<typename K, typename V, typename Hash, typename Index, typename Equal, int RehashingFactor>
requires RehashingFactorConstraint<RehashingFactor>bool
hash_table<K, V, Hash, Index, Equal, RehashingFactor>::find(K key) {
    uint256 hash_code = hash(key);
    int i = (int) (hash_code % b);
    std::list<entry> &bucket = buckets[i];
...
template<typename K, typename V, typename Hash, typename Index, typename Equal, int RehashingFactor>
requires RehashingFactorConstraint<RehashingFactor>bool hash_table<K, V, Hash, Index, Equal, RehashingFactor>::empty() {
    return key_count() == 0;
}

template<typename K, typename V, typename Hash, typename Index, typename Equal, int RehashingFactor>
requires RehashingFactorConstraint<RehashingFactor>int hash_table<K, V, Hash, Index, Equal, RehashingFactor>::size() {
    return this->v;
}
...
requires RehashingFactorConstraint<RehashingFactor>
hash_table<K, V, Hash, Index, Equal, RehashingFactor>::hash_table(
        Index index, Hash hash, Equal equal, int b, float max_load_factor
) : index(index),
    b(b),
    v(0),
    k(0),
